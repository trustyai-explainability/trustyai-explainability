...
class KMeansGeneratorTest {
    ...
    void testKMeansGeneration(int clusters) {
        ...
        assertTrue(background.stream().allMatch(pi ->
                expectedCentroids.contains(pi.getFeatures().get(0).getValue().asNumber())));
    }
    ...
    void testKMeansGaussian(int clusters) {
        ...
        assertTrue(background.stream().allMatch(pi -> expectedCentroids.stream().anyMatch(o ->
                Math.sqrt(Math.pow(o - pi.getFeatures().get(0).getValue().asNumber(), 2))<.05)));
    }
}