...
import org.kie.trustyai.explainability.utils.RandomChoice;
import org.kie.trustyai.explainability.utils.WeightedLinearRegression;
import org.kie.trustyai.explainability.utils.WeightedLinearRegressionResults;
...
public class ShapKernelExplainer implements LocalExplainer<SaliencyResults> {
    ...
    private ShapDataCarrier initialize(PredictionProvider model) {
        ...
        CompletableFuture<List<PredictionOutput>> rawNull = model.predictAsync(config.getBackground());
        CompletableFuture<RealMatrix> modelNull = rawNull
        ...
        CompletableFuture<Map<String, Double>> nullOutput = rawNull.thenCombine(linkNull, (rn, ln) -> {
            Map<String, Double> result = new HashMap<>();
            for (int i = 0; i < ln.getDimension(); i++) {
                result.put(rn.get(0).getOutputs().get(i).getName(), ln.getEntry(i));
            }
            return result;
        });
