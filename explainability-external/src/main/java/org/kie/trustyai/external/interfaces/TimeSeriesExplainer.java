...
public interface TimeSeriesExplainer<T> {
    ...
    default CompletableFuture<T> explainAsync(TsFrame dataframe, PredictionProvider model) {
        return explainAsync(dataframe, model, unused -> {
            /* NOP */
        });
    };

    CompletableFuture<T> explainAsync(TsFrame dataframe, PredictionProvider model, Consumer<T> intermediateResultsConsumer);
